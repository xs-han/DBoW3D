...
//	}
	...
	fidx2kpsidx[fidx] = (int)_features.rows;
	kpsidx2fidx[(int)_features.rows] = fidx;
	if(getfeatures)
...
const cv::Mat& View::getFeatures() const {
	return _features;
}

const CameraT& View::getC() const {
	return _C;
}

void View::setC(const CameraT& c) {
	_C.SetCameraT(c);
}
